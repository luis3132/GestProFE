...
const AdminNavbar: FC<AdminNavbarProps> = ({ admin, changeRole }) => {
    ...
    return (
        <nav className={`flex flex-col bg-green-500 justify-around items-center w-full shadow-xl ${showMenu ? "h-48" : "h-20"} duration-500 overflow-hidden max-md:pt-2`}>
            <div className="flex justify-between items-center w-full max-md:mb-4">
                <Link href={"/admin"} className="flex items-center ml-3 rounded-2xl shadow-2xl py-1 px-2 hover:bg-green-600">
                    <Image src="/gestpro.png" alt="GestPro" width={50} height={50} />
                    <h1 className="text-white text-2xl font-bold">GestPro</h1>
                </Link>
                {admin && <Link href={"/home"} className="flex items-center h-12 rounded-lg shadow-xl text-lg text-white hover:bg-green-600 px-4 md:hidden" onClick={changeRole}>
                ...
                <div className="flex items-center max-md:hidden mr-3">
                    {admin && <Link href={"/home"} className="flex items-center h-12 rounded-lg shadow-xl text-lg text-white hover:bg-green-600 px-4" onClick={changeRole}>
                        <Icon icon="icon-park-outline:change" width="30" height="30" className="font-bold mr-1" />
                        <p className="font-bold">Cambiar a Usuario</p>
                    </Link>}
